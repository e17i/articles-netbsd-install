...
# (%e)</p><p>Made on emacs org-mode with <a
# href=\"https://jessekelly881-rethink.surge.sh/\">Rethink</a></p>"));
...
#+EMAIL: e17i#uni-bremen.de
#+TITLE: Installation of NetBSD on a Mac Mini

* What's the problem?

Over the easter days I was planning to resurrect my old ppc based Mac Mini and
to replace the old osx 10.4 by a newer, but still BSD-based operating system.
The first try was to install OpenBSD, which went quite smooth and without
great hassles. Fine.
...
portions until you find the missing detail. The four major points for me were:
  ...
  - When partitioning and formatting the disk, the ~sysinst~ tool generates
    ...
    then partitioned using bsd disklabels. NetBSD for macppc seems to need all partitions to be
    ...
    adjacent =/etc/fstab=).
  - When doing the first reboot after installing the base sets, you find that
  ...
  - Finally, your system boots, but stays with a white screen. This is because
    ...
    screen before loading the boot program, this way activating the console.
